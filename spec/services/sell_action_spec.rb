...
RSpec.describe SellAction, type: :service do
  ...
  let(:game_resource) { create(:game_resource, game: game, resource: resource, current_price: 15.0) }
  ...
  describe '#valid?' do
    ...
    context 'with insufficient inventory' do
      let(:action) { described_class.new(game, game_resource_id: game_resource.id, quantity: 50) }
    ...
    context 'when player does not own the resource' do
      ...
      let(:other_game_resource) { create(:game_resource, game: game, resource: other_resource, current_price: 15.0) }
      let(:action) { described_class.new(game, game_resource_id: other_game_resource.id, quantity: 1) }
  ...
  describe '#run' do
    ...
    before do
      # Update game_resource to have a selling price of $20
      game_resource.update!(current_price: 20.0)
    ...
    context 'with multiple inventory stacks at different prices (FIFO)' do
      let(:action) { described_class.new(game, game_resource_id: game_resource.id, quantity: 15) }

      before do
        # Update game_resource to have a selling price of $25
        game_resource.update!(current_price: 25.0)
